...
import { addExposingStep } from 'Store/Reducers/Exposing/ExposingFlow/ExposingSlice';
import { addStep } from 'Store/Reducers/Editing/EditingFlow/EditingSlice';
...
const UpdateInquity = () => {
  ...
  const { inquiryLoading, selectedInquiryFlowData, getInquiryStepData } =
  ...
  const loadProductAndPackageData = (type = values?.inquiry_type) => {
    dispatch(
      getPackageList({
        start: 0,
        limit: 0,
      ...
      .then(({ packageData }) => {
        dispatch(
          ...
          .then(response => {
            let productData = [];
            // let updatedInquiryData = { ...inquiryResponse };

            // if (inquiryResponse?.inquiry_type === 2) {
            //   const updatedExposingTableData =
            //     inquiryResponse?.exposing_inquiryTable?.map(item => {
            ...
            //   updatedInquiryData = {
            //     ...updatedInquiryData,
            //     exposing_inquiryTable: updatedExposingTableData,
            //   };
            // }
            ...
            // dispatch(setSelectedInquiryFlowData(updatedInquiryData));
    ...
    return (
      <div className="form_group date_select_wrapper w_260 hover_date">
        {/* <Calendar
          ...
          showIcon
          dateFormat="dd-mm-yy"
          value={rowData?.order_date ? new Date(rowData?.order_date) : ''}
          name="order_date"
          minDate={new Date(values?.create_date)}
          readOnlyInput
          onChange={e => {
            const utcDate = e.value
              ? //  new Date(e.value)
                getFormattedDate(e.value)
              : '';
            handleExposingTableChange(rowData, e.target.name, utcDate);
          }}
          showButtonBar
        /> */}
        ...
        if (id) {
          ...
          dispatch(editInquiry(payload))
            .then(res => {
              dispatch(getInquiry({ inquiry_id: id }))
                .then(res => {
                  const inquiryResponse = res?.payload;
                  dispatch(setSelectedInquiryFlowData(inquiryResponse));
                  if (
                    !getInquiryStepData?.inquiry_step ||
                    getInquiryStepData?.inquiry_step < 1
                  ) {
                    let payload = {
                      inquiry_id: id,
                      is_rework: false,
                    ...
                    dispatch(addInquiryStep(payload))
                      .then(response => {
                        ...
                        values?.inquiry_type === 1
                          ...
                          : dispatch(addExposingStep(addStepPayload));
                      })
                      .catch(errors => {
                        console.error('Add Inquiry status:', errors);
                      });
                  }
                })
                .catch(error => {
                  console.error('Error fetching Inquiry Flow:', error);
                });
            })
            .catch(error => {
              console.error('Error fetching while edit inquiry form:', error);
            });
        }
      }
    },
    [id, dispatch, getInquiryStepData],
  );
  ...
  const {
    ...
    initialValues: selectedInquiryFlowData,
...
export default memo(UpdateInquity);
