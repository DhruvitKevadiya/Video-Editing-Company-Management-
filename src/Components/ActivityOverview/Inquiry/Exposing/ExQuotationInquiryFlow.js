...
const ExQuotationInquiryFlow = () => {
  ...
  const { getInquiryStepData, inquiryFlowExQuotationData } = useSelector(
    ({ inquiry }) => inquiry,
  );
  ...
  useEffect(() => {
    if (!inquiryFlowExQuotationData?.quotation_id) {
        ...
        .then(({ quotationName }) => {
          dispatch(getExposingDetails({ order_id: id }))
            ...
            .then(async ({ responseData }) => {
              ...
              let updatedList = responseData?.exposing_order_table?.map(d => {
  ...
  }, [dispatch, getInquiryStepData, id, selectedExposingQuatationData]);
          ...
          sub_total: values?.total_amount_collection
            ? convertIntoNumber(
                values?.total_amount_collection *
                  currentCurrencyRate?.exchange_rate,
              )
            : 0,
        ...
        if (isEdit) {
          dispatch(editQuotation(payload))
            .then(async response => {
              resetForm();
              setIsEdit(false);
              const updatedExposingQuotationInquiryData =
                await fetchRequiredData(values);
              ...
              if (selectedExposingQuatationData?.status === 2) {
                ...
                dispatch(
                  setInquiryFlowExQuotationData(
                    updatedExposingQuotationInquiryData,
                  ),
                );
              }
  ...
  const {
    ...
    initialValues: inquiryFlowExQuotationData,
...
export default memo(ExQuotationInquiryFlow);
